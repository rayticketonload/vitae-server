  ...
  PM.wxAddNewPack(
    {
      ...
      parent_id:
        (req.body.parentId && req.body.parentId.trim().toString()) || null,
      image_path:
        (req.body.imagePath && req.body.imagePath.trim().toString()) || '',
    },
    function(e, data) {
      if (e) {
        res.status(200).send({
  ...
  PM.updatePackInfo(
    {
      ...
      parentId:
        (req.body.parentId && req.body.parentId.toString().trim()) || null,
      id: (req.body.id && req.body.id.toString().trim()) || null,
      imagePath:
        (req.body.imagePath && req.body.imagePath.trim().toString()) || '',
    },
...
exports.deletePackById = (req, res) => {
  ...
  var goods = [];
  PM.getUserAllPackListById(
    { user_id: req.user.user_id, id: req.body.id },
    function(err, data) {
      if (err)
        res.status(200).send({
          code: 100,
      ...
      if (data.length > 0) {
        ...
        GM.getUserAllPackListByPackList(
          ...
          function(err, data) {
            ...
            PM.deletePackByList(
              { list: packs.map(item => item.id), user_id: req.user.user_id },
              function(err, data) {
                if (!err) {
                  GM.deleteGoodByList(
                    {
                      list: goods.map(item => item.id),
                      user_id: req.user.user_id,
                    },
                    function(err, data) {
                      if (!err) {
...
/**
 * 获取盒子内所有盒子列表
 * (例如所传盒子ID为1，就只获取parent_id为1的盒子集合)
 * @param {*} req
 * @param {*} res
 */
